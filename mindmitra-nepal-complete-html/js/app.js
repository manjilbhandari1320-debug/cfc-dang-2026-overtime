...
const configured=window.MINDMITRA_SUPABASE_URL&&window.MINDMITRA_SUPABASE_ANON_KEY&&!window.MINDMITRA_SUPABASE_URL.includes('YOUR_PROJECT_REF')&&!window.MINDMITRA_SUPABASE_ANON_KEY.includes('YOUR_SUPABASE');
const sb=configured?window.supabase.createClient(window.MINDMITRA_SUPABASE_URL,window.MINDMITRA_SUPABASE_ANON_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}}):null;
...
async function saveAssessmentResult(){
  if(!sb || !currentUser || !assessmentState.result) return;
  ...
  const answerRows = ASSESSMENT_QUESTIONS.map((question, index) => ({
    session_id: sessionRow.id,
    question_key: `mental_health_${index + 1}`,
    answer: {
  ...
  answerRows.push({
    ...
    answer: assessmentState.lifestyle
  });
  ...
  const riskMap = {
    ...
    "Moderate Concern": "elevated",
    "High Concern": "urgent"
  };

  const { error: reportError } = await sb.from("ai_reports").insert({
    session_id: sessionRow.id,
    user_id: currentUser.id,
    user_summary: {
      level: assessmentState.result.level,
      score: assessmentState.result.score,
      message: assessmentState.result.message,
      suggestions: assessmentState.result.suggestions
    },
    counsellor_summary: {
      score: assessmentState.result.score,
      category: assessmentState.result.level,
      lifestyle: assessmentState.lifestyle
    },
    risk_level: riskMap[assessmentState.result.level],
    ...
    <div class="assessment-shell">
      ...
      <div class="suggestion-card">
        ...
        <div class="assessment-nav-row">
          ...
          ${result.score >= 30 ? '<button id="assessment-counsellor" class="btn-gold">Find a counsellor</button>' : '<button id="assessment-dashboard" class="btn-gold">Back to dashboard</button>'}
        </div>
      </div>
    </div>
  `;
...
function card(c,showDistance=false){const dist=showDistance&&Number.isFinite(c.distance_km)?`<div class="distance-pill">📍 ${formatDistance(c.distance_km)}</div>`:'';return `<article class="public-counsellor-card"><div class="counsellor-avatar">${(c.full_name||'C').charAt(0)}</div><div class="flex-1"><div class="flex items-start justify-between gap-3"><div><h3>${c.full_name||'Verified Counsellor'}</h3><p class="credential">${c.professional_title||'Counsellor'}</p></div><span class="verified-badge">✓ Verified</span></div><p class="speciality-line">${(c.specialities||[]).join(' · ')||'General wellness support'}</p><p class="address-line">📌 ${[c.address_line,c.city,c.district,c.province].filter(Boolean).join(', ')}</p><p class="language-line">Languages: ${(c.languages||[]).join(', ')||'Not specified'}</p>${dist}</div></article>`}

async function fetchCounsellors(){if(!sb)return demoCounsellors;const {data,error}=await sb.from('counsellor_profiles').select(`user_id,professional_title,specialities,languages,address_line,city,district,province,latitude,longitude,profiles!inner(full_name)`).eq('verification_status','approved');if(error)throw error;return (data||[]).map(x=>({...x,full_name:x.profiles?.full_name||'Verified Counsellor'}))}
async function fetchNearby(lat,lng,radius=250){if(!sb)return demoCounsellors.map(c=>({...c,distance_km:distanceKm(lat,lng,c.latitude,c.longitude)})).filter(c=>c.distance_km<=radius).sort((a,b)=>a.distance_km-b.distance_km);const {data,error}=await sb.rpc('find_nearby_counsellors',{user_lat:lat,user_lng:lng,radius_km:radius});if(error)throw error;return data||[]}
...
function updateAuth(){$$('[data-auth-role]').forEach(b=>b.classList.toggle('active',b.dataset.authRole===role));$$('[data-auth-mode]').forEach(b=>b.classList.toggle('active',b.dataset.authMode===mode));const restricted=role==='client';$('[data-auth-mode="signup"]').classList.toggle('hidden',restricted);$('#restricted-note').classList.toggle('hidden',!restricted);$('#signup-fields').classList.toggle('hidden',mode!=='signup');$('#org-type-wrap').classList.toggle('hidden',!(mode==='signup'&&role==='organization_admin'));$('#counsellor-extra').classList.toggle('hidden',!(mode==='signup'&&role==='counsellor'));$('#first-login-note').classList.toggle('hidden',role!=='client');$('#anonymous-choice').classList.toggle('hidden',role!=='client');$('#auth-submit').textContent=mode==='signup'?'Create Account':'Sign In';clearAuthError()}

async function register(){const email=$('#auth-email').value.trim(),password=$('#auth-password').value,full_name=$('#full-name').value.trim();if(!full_name)throw new Error('Enter your full name or organization name.');if(password.length<8)throw new Error('Password must have at least 8 characters.');const metadata={full_name,role};if(role==='organization_admin'){metadata.organization_name=full_name;metadata.organization_type=$('#org-type').value}if(role==='counsellor'){metadata.phone=$('#counsellor-phone').value.trim();metadata.licence_number=$('#licence-number').value.trim();metadata.address_line=$('#counsellor-address').value.trim();metadata.city=$('#counsellor-city').value.trim();metadata.province=$('#counsellor-province').value.trim();metadata.latitude=$('#counsellor-latitude').value||null;metadata.longitude=$('#counsellor-longitude').value||null;if(!metadata.phone||!metadata.licence_number||!metadata.address_line||!metadata.city)throw new Error('Counsellors must provide contact, licence and address details.')}const {data,error}=await sb.auth.signUp({email,password,options:{data:metadata}});if(error)throw error;if(data.user&&role==='counsellor'){const {error:e}=await sb.from('counsellor_profiles').upsert({user_id:data.user.id,phone:metadata.phone,licence_number:metadata.licence_number,address_line:metadata.address_line,city:metadata.city,province:metadata.province,latitude:metadata.latitude?Number(metadata.latitude):null,longitude:metadata.longitude?Number(metadata.longitude):null,verification_status:'pending'});if(e)throw e}toast('Account created. Check your email if verification is enabled.','success');mode='signin';updateAuth()}


async function requirePasswordChangeIfNeeded(profile){
  if(!profile?.first_login) return false;

  const newPassword = window.prompt(
    "This is your first login. Create a new private password with at least 8 characters:"
  );

  if(!newPassword) throw new Error("You must change the temporary password before continuing.");
  if(newPassword.length < 8) throw new Error("Your new password must contain at least 8 characters.");

  const { error: passwordError } = await sb.auth.updateUser({ password: newPassword });
  ...
  toast("Your new password has been saved.", "success");
  return true;
}

async function signin(){const email=$('#auth-email').value.trim(),password=$('#auth-password').value;const {data,error}=await sb.auth.signInWithPassword({email,password});if(error)throw error;const {data:p,error:e}=await sb.from('profiles').select('id,full_name,role,organization_id,first_login').eq('id',data.user.id).single();if(e)throw e;if(p.role!==role){await sb.auth.signOut();throw new Error(`This account belongs to the ${p.role} portal.`)}
await requirePasswordChangeIfNeeded(p);
currentUser=data.user;
currentProfile=p;if(role==='client'){anonymousMode=$('#anonymous-toggle').checked;const alias=$('#anonymous-alias').value.trim()||`Mitra-${data.user.id.slice(0,6).toUpperCase()}`;await sb.from('privacy_preferences').upsert({user_id:data.user.id,share_identity_with_counsellor:!anonymousMode,anonymous_alias:alias},{onConflict:'user_id'})}enterApp(p.full_name||email)}

$('#auth-form').onsubmit=async e=>{e.preventDefault();clearAuthError();const email=$('#auth-email').value.trim(),password=$('#auth-password').value;if(!email||!password)return showError('Enter your email and password.');if(!sb)return showError('Add Supabase URL and anon key, or use the demo button.');setLoading(true);try{mode==='signup'?await register():await signin()}catch(e){showError(e.message)}finally{setLoading(false)}};
$('#demo-login').onclick=()=>{anonymousMode=$('#anonymous-toggle').checked;enterApp(labels[role]+' Demo')};

async function restore(){if(!sb)return;const {data:{session}}=await sb.auth.getSession();if(!session)return;const {data:p}=await sb.from('profiles').select('id,full_name,role,organization_id,first_login').eq('id',session.user.id).maybeSingle();if(!p||false)return;role=p.role;currentUser=session.user;currentProfile=p;if(role==='client'){const {data:pr}=await sb.from('privacy_preferences').select('*').eq('user_id',p.id).maybeSingle();anonymousMode=pr?!pr.share_identity_with_counsellor:false}enterApp(p.full_name||session.user.email||'User')}
...
$('#logout').onclick=async()=>{if(sb)await sb.auth.signOut();currentUser=null;currentProfile=null;$('#role-app').classList.add('hidden');$('#public-site').classList.remove('hidden');toast('Logged out safely.','success')};
...
$('#contact-form').onsubmit=async e=>{e.preventDefault();const payload={name:$('#contact-name').value.trim(),email:$('#contact-email').value.trim(),topic:$('#contact-topic').value,message:$('#contact-message').value.trim()};try{if(sb){const {error}=await sb.from('contact_messages').insert(payload);if(error)throw error}e.target.reset();toast('Thank you. Your message has been received.','success')}catch(err){toast(err.message,'error')}};
...
if(page==='dashboard'){if(role==='client')return stats([['Assessment','65%'],['Latest summary','Balanced'],['Appointment','1'],['Privacy',anonymousMode?'Anonymous':'Identified']])+panel('Next steps','<div class="next-step-list"><button data-jump="assessment">Continue assessment</button><button data-jump="counsellors">Browse nearby counsellors</button><button data-jump="notifications">Review reminder preferences</button></div>');if(role==='counsellor')return stats([['Pending requests','6'],['Confirmed today','3'],['Authorized reports','9'],['Anonymous clients','4']])+panel('Recent requests',table(['Client display','Concern','Appointment','Action'],[['Mitra-104','Moderate','Requested','<button class="action">Review</button>'],['Aarav S.','General support','Confirmed','<button class="action">Open</button>']]));return stats([['Registered users','486'],['Participation','74%'],['Completion','62%'],['Verified counsellors','12']])+panel('Organization Admin access','Create users securely, manage groups and view anonymous participation analytics. Private assessment answers remain hidden.')}
...
$$('.create-meet').forEach(button=>{
  button.onclick=async()=>{
    ...
    try{
      ...
      const response=await fetch(`${window.MINDMITRA_SUPABASE_URL}/functions/v1/create-google-meet`,{
        method:'POST',
        headers:{
      ...
      if(!response.ok) throw new Error(result.error||'Unable to create Google Meet.');
  ...
  $$('.send-followup').forEach(button=>{
    button.onclick=async()=>{
      ...
      try{
        if(sb&&currentUser){
          const {data:{session}}=await sb.auth.getSession();
          const response=await fetch(`${window.MINDMITRA_SUPABASE_URL}/functions/v1/send-counsellor-followup`,{
            method:'POST',
            headers:{
              'Content-Type':'application/json',
              'Authorization':`Bearer ${session.access_token}`
          ...
          const result=await response.json();
          if(!response.ok) throw new Error(result.error||'Unable to send follow-up.');
        }
        toast(`Follow-up email sent to ${clientDisplay}.`,'success');
  ...
  <div class="appointment-list">
    <article class="appointment-card">
      ...
      <div class="appointment-actions">
        <button class="action create-meet" data-appointment="demo-appointment-3">Create Google Meet</button>
        <a class="action secondary hidden meet-link" target="_blank" rel="noopener">Join meeting</a>
      </div>
    </article>
    <article class="appointment-card">
      <div>
...
return panel((nav[role].find(x=>x[0]===page)||['','','Module'])[2],'<div class="empty-state"><div>🌿</div><h3>This module is ready</h3><p>Connect it to the matching Supabase table.</p></div>');
}

async function loadPanel(list=null,dist=false){const g=$('#counsellor-list');if(!g)return;try{const items=list||await fetchCounsellors();g.innerHTML=items.length?items.map(c=>card(c,dist)).join(''):'<div class="empty-state col-span-full"><div>🩺</div><h3>No counsellors found</h3></div>'}catch(e){g.innerHTML=`<div class="empty-state col-span-full"><div>!</div><h3>${e.message}</h3></div>`}}
...
if(page==='assessment'){
  ...
  if(finish) finish.onclick=async()=>{
    ...
    try{
      ...
      if(sb&&currentUser) toast('Assessment saved securely.','success');
    }catch(error){
...
if(page==='privacy')$('#save-privacy').onclick=async()=>{anonymousMode=$('#panel-anonymous').checked;const alias=$('#panel-alias').value.trim()||null;try{if(sb&&currentUser){const {error}=await sb.from('privacy_preferences').upsert({user_id:currentUser.id,share_identity_with_counsellor:!anonymousMode,anonymous_alias:alias||`Mitra-${currentUser.id.slice(0,6).toUpperCase()}`},{onConflict:'user_id'});if(error)throw error}$('#user-badge').textContent=anonymousMode?'A':userName.charAt(0).toUpperCase();toast('Privacy preference saved.','success')}catch(e){toast(e.message,'error')}};
...
if(page==='create-user')$('#create-org-user-form').onsubmit=async e=>{e.preventDefault();const s=$('#create-user-status');s.textContent='Creating account…';try{if(!sb||!currentUser){s.textContent='Demo mode: user created.';toast('Demo user created.','success');e.target.reset();return}const {data:{session}}=await sb.auth.getSession();const response=await fetch(`${window.MINDMITRA_SUPABASE_URL}/functions/v1/create-organization-user`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${session.access_token}`},body:JSON.stringify({full_name:$('#member-name').value.trim(),email:$('#member-email').value.trim(),username:$('#member-username').value.trim(),temporary_password:$('#member-password').value,member_type:$('#member-type').value,department:$('#member-department').value.trim(),allow_anonymous_counselling:$('#allow-anonymous').checked})});const r=await response.json();if(!response.ok)throw new Error(r.error||'Unable to create user.');s.textContent=`Account created for ${r.email}`;toast('Organization user created.','success');e.target.reset()}catch(err){s.textContent=err.message;toast(err.message,'error')}};
}
